...
public class ControlsH {
    ...
    public static void CreateColumns (DataGrid dataGrid, Type type, double font_size = 14) {
        var ps = type.GetProperties();
        for (int i = 0; i < ps.Length; i++) {
            var p = ps[i];
            CreateColumn(dataGrid, p.Name, 1, DataGridLengthUnitType.Auto, font_size);
        }
    }
    ...
    public static void CreateColumn(DataGrid datagrid, string header, int width = 1, DataGridLengthUnitType type = DataGridLengthUnitType.Star, double font_size = 12) {
